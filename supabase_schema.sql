...
-- ============================================================================
-- 29. DEDICATED ADMIN & TRAINER SECURE AUTHENTICATION TABLES
-- ============================================================================

-- Admin Credentials Table (Secure platform governance)
...
-- Dedicated Trainer Logins Table (Stores approved trainers' login credentials)
...
-- Seed Admin credentials into Supabase (username: pathfinders, password: 123456, role: admin)
...
-- Ensure Admin also exists in auth.users and profiles
DO $$
DECLARE
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'pathfinders@pathfinder.org' OR id = v_admin_id) THEN
    INSERT INTO auth.users (
      ...
      'pathfinders@pathfinder.org', crypt('123456', gen_salt('bf')), now(),
  ...
  VALUES (v_admin_id, 'PathFinders Administrator', 'pathfinders', 'pathfinders@pathfinder.org', 'admin', 'active')
  ON CONFLICT (id) DO UPDATE
  SET username = 'pathfinders', role = 'admin', status = 'active', updated_at = now();
END $$;

-- RPC: Verify Platform Login (Admin, Trainer) without exposing credentials in client code
...
-- RPC: Record Approved Trainer Login (Called whenever Admin approves a trainer)
